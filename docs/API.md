...
1. **폴링 폐기 → WebSocket.** 채팅방마다 실시간 연결을 맺고, 같은 연결로 상대가
   방에 들어와 있는지(presence)도 표시한다. 라이브러리는 미정 — 이 문서는
   이벤트 계약(§3)만 고정한다.
...
7. **역할 재편.** A = 인증·헤더·사이드바 영역 풀스택, B = 메인 기능 프론트엔드,
   C = 메인 기능 백엔드. 상세는 `ARCHITECTURE.md` §6.
...
|---|---|---|---|---|
| GET | `/login` | ✕ | A | 로그인 폼 |
| GET | `/signup` | ✕ | A | 회원가입 폼. 아이디·비밀번호·이름 + **소속 4종 선택** |
| POST | `/signup` | ✕ | A | 가입 처리 → 성공 시 `302 /login`. 아이디 중복이면 폼 재렌더+에러 |
| GET | `/logout` | ○ | A | `token` 쿠키 삭제 → `302 /login` (+"로그아웃되었어요" 안내) |
| GET | `/` | ○ | B/C | 홈 피드. **`?page=N` 페이지네이션, 페이지당 최대 20개, 하단 페이지 번호.** 유형 필터 칩과 조합 시 `?type=sale&page=2`. 완료 글은 '완료' 뱃지 + 사진 흑백 |
| GET | `/search` | ○ | A | **검색 결과.** `?q=<검색어>`로 items 제목을 부분일치 검색, 피드와 같은 카드로 렌더. (판매자 이름 검색 등 확장 범위는 미정) |
| GET | `/items/new` | ○ | B/C | 거래 글 작성 폼 |
| POST | `/items` | ○ | B/C | 글 등록(multipart, 사진 1장) → `302 /` |
| GET | `/items/<id>` | ○ | B/C | **물품 상세.** 사진 · 제목 · 가격 · 유형 뱃지 · 상태 뱃지 · 판매자(이름·소속) · 설명. 판매자 본인이면 **삭제 버튼 + 거래완료 토글**이 추가로 그려진다 |
| GET | `/items/<id>/chat` | ○ | B/C | **채팅 입구.** 구매자면 (글, 나) 방을 찾거나 생성 → `302 /chats/<room_id>`. 판매자면 그 글의 첫 번째 방으로 `302`. 방이 없으면 `302 /items/<id>` + "아직 문의가 없어요" |
| POST | `/items/<id>/delete` | ○ 판매자만 | B/C | 글 삭제 → `302 /` |
| GET | `/chats` | ○ | B/C | 내가 참여한 1:1 방 목록. **헤더의 말풍선 아이콘이 여기로 연결**된다(아이콘 자체는 A의 헤더 몫) |
| GET | `/chats/<room_id>` | ○ 방 당사자만 | B/C | 1:1 채팅. 상단에 물건 요약 한 줄. 입장하면 WebSocket 연결(§3). 뷰어가 판매자면 하단에 같은 글의 방 페이지네이션 |
| GET | `/history` | ○ | A | 거래내역. 판매 탭(P0) / 구매 탭(P1) |
| GET/POST | `/items/<id>/edit` | ○ 판매자만 | 공용 풀 | **P1** 글 수정 |
| GET | `/community` `/community/new` `/community/<id>` | ○ | A | **P2** 커뮤니티 3종 (거래 CRUD 복사). 엔드포인트 설계·구현은 A |

> B/C가 함께 적힌 행: C가 라우트(백엔드), B가 그 화면의 템플릿·JS(프론트엔드)를 맡는다.
> 파일 단위 경계는 미확정 — 제안은 `ARCHITECTURE.md` §6.
>
...
## 3. WebSocket 이벤트 계약 (초안 — 라이브러리 확정 후 경로 표기만 조정)
...
- **미정**: 라이브러리(Flask-SocketIO vs flask-sock 등). 결정되면 연결 경로
  (`/ws/rooms/<id>` vs SocketIO room join)와 이벤트 표기법만 이 표에 맞춰 조정한다.
