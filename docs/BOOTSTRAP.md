...
|---|---|---|---|---|
| `navbar` + 검색 폼 | 헤더 (햄버거·로고·검색·말풍선) | 공식 마크업 + `<form action="/search">` | A | 하 |
| `offcanvas` | 햄버거 사이드바 | data 속성만, **JS 0줄** | A | 하 |
| `card` | 사이드바 메뉴, 피드 카드 | 클래스만 | A / B | 하 |
| `row-cols-1 row-cols-md-3` | 피드 그리드 | 클래스 하나로 3열↔1열 반응형 | B | 하 |
| `badge` | 유형·상태 뱃지 ('완료' 포함) | 클래스만 | B | 하 |
| `alert` | "로그아웃되었어요" 안내 | 클래스만 | A | 하 |
| `form-control` | 모든 입력 폼 | 클래스만 | 전원 | 하 |
| `nav-pills` / `nav-tabs` | 필터 칩·거래내역 탭 | 탭 전환도 `data-bs-toggle="tab"` 선언만 | B / A | 하 |
| `pagination` | **홈 피드 하단** + 판매자 방 번호 | 클래스만 (Jinja 반복) | B | 하 |
| `btn-check` (라디오) | 거래 유형·**소속 4종** 선택 | 패턴 복붙 + (유형만) 필드 분기 JS | B / A | 중 |
| 완료 카드 흑백 | 피드·거래내역의 done 글 | CSS `filter: grayscale(1)` 한 줄 | B | 하 |

**우리가 직접 짜는 JS는 ws.js(WebSocket 클라이언트) · 거래완료 토글 · 유형 필드 분기(P1)뿐이다.**
나머지는 전부 "HTML에 속성을 쓰면 Bootstrap JS가 알아서 동작"하는 선언형이다.

## 2. 핵심 스니펫

### 헤더 (navbar + 검색 + 말풍선 + 햄버거) — [A]
...
```
```css
/* 커스텀 CSS 최소화 방침의 예외 한 줄 — Bootstrap에 흑백 유틸리티가 없다 */
.card-done img { filter: grayscale(1); }
```

### 홈 피드 페이지네이션 — [B], 페이지당 20개
...
### 거래완료 토글 + 삭제 (물품 상세, 판매자만) — 버튼은 [B], API는 [C]
...
### 거래 유형 선택 (btn-check) + 필드 분기 — [B], 분기는 P1
...
### presence 표시 (채팅 화면) — [B], 데이터는 ws.js의 presence 이벤트

```html
<span id="peer-presence" class="badge text-bg-success" hidden>상대방이 보고 있어요</span>
```
```js
// ws.js — presence 이벤트 핸들러에서
onPresence = ({online}) => document.getElementById('peer-presence').hidden = !online;
```
...
4. **base.html은 A의 파일이다.** B·C는 base를 고치고 싶으면 A에게 말한다 —
   세 브랜치가 같은 파일을 건드리는 순간 머지 충돌이 시작된다.
