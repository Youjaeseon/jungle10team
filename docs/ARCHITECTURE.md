...
- **상태의 주인은 `items.status` 한 곳.** 피드·채팅 화면·내 거래글의 뱃지가 전부 여기를 읽는다.
...
| 담당 | 축 | 영역 |
|---|---|---|
| **진근** (A) | 영역 풀스택 | 로그인·회원가입(JWT 발급·쿠키 저장, 소속 4종 선택)·로그아웃 · **헤더**(검색 UI + DB 연동 검색 기능, 말풍선 아이콘, 햄버거) · **사이드바**(offcanvas + 카드 메뉴) · 사이드바에서 이어지는 **내 거래글 · 로그아웃 · 커뮤니티**의 화면과 엔드포인트 전부 |
...
| 2일차 | 검색(/search) + 내 거래글(/history) | ws.js + 채팅 화면 + 완료 흑백·토글 UI | 방 찾기/생성 + SocketIO 서버(메시지·presence) + status 토글 API |
